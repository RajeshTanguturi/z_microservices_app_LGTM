  ...
  datasources.yaml:
    ...
    - name: Tempo
      ...
      jsonData:
        ...
        tracesToLogs:
          datasourceUid: 'Loki'
          tags: ['k8s.pod.name', 'service.name']
EOF
...
COLLECTOR_URL="otel-collector-opentelemetry-collector.monitoring.svc.cluster.local:4317"

for deploy in $(kubectl get deploy -n demo -o jsonpath='{.items[*].metadata.name}'); do
  kubectl set env deployment/$deploy -n demo \
    ENABLE_TRACING="1" \
...
echo "Fetching External IPs (press Ctrl+C when assigned)..."
